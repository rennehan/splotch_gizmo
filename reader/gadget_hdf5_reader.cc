                            ...
                            unsigned int *nparttotal, double &boxsize, paramfile &params)
{
  ...
  read_hdf5_group_attribute(hdf5_header, "Time",         &time,      H5T_NATIVE_DOUBLE);
...
//            filename+="."+dataToString(ThisTaskReads[ThisTask]+f)+".hdf5";
          ...
          for(int itype=0; itype<ptypes; itype++)
          {
            ...
            hid_t group_id;
            group_id = H5Gopen(file_id, GroupName.c_str(), H5P_DEFAULT);
            readHDF5DataArray(group_id, "Velocities", H5T_NATIVE_FLOAT, (void*) &ftmp[0]);
            H5Gclose(group_id);
...
//        filename+="."+dataToString(ThisTaskReads[ThisTask]+f)+".hdf5";
      ...
      for(int itype=0; itype<ptypes; itype++)
      {
        ...
        if ((read_col>0) && label_col == to_check) 
          fnread=11*npartthis[type];
        ...
        if(mpiMgr.master() && !params.find<bool>("AnalyzeSimulationOnly"))
          cout << " Reading " << label_col.c_str() << endl;
